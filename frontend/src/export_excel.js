...
const exportExcel = async function (luckysheet) { // 参数为luckysheet.getluckysheetfile()获取的对象
  ...
  luckysheet.every(function (table) {
    ...
    setRowLength(table.config.rowlen, worksheet)
    setColumnLength(table.config.columnlen, worksheet)
    return true
  })
  // 4.写入 buffer
...
var fillConvert = function (bg) {
  if (!bg) {
    return { type: 'pattern', pattern: 'none', bgColor: { argb: 'FFFFFFFF' } }
  }
  const fill = {
    type: 'pattern',
    pattern: 'solid',
...
var setRowLength = function (rows, sheet) {
  if (rows && Object.keys(rows).length > 0) {
    for (const rowIndex in rows) {
      const row = sheet.getRow(parseInt(rowIndex) + 1)
      row.height = parseFloat(rows[rowIndex]) / 2
    }
  }
}

var setColumnLength = function (cols, sheet) {
  if (cols && Object.keys(cols).length > 0) {
    for (const colIndex in cols) {
      const col = sheet.getColumn(parseInt(colIndex) + 1)
      col.width = parseFloat(cols[colIndex]) / 2.54
    }
  }
}
...
var borderConvert = function (borderType, style = 1, color = '#000') { // 对应luckysheet的config中borderinfo的的参数
  ...
  const template = { style: luckyToExcel.style[style], color: { argb: color.replace('#', '') } }
  const border = {}
